...
def load_offline_shap_importance(path: str) -> dict:
    """Load mean-|SHAP| feature importance generated by shap_explain.py.

    This makes the offline SHAP result visible in Prometheus immediately,
    even before any online fraud messages containing top_shap_features arrive.
    """
    if not path:
        return {}
    ...
    p = Path(path)
    if not p.exists():
        logger.warning("Offline SHAP file not found at %s", p)
        return {}
    ...
    try:
        ...
        for feature, value in raw.items():
            try:
                ...
                continue
        logger.info("Loaded %d offline SHAP feature importances from %s", len(out), p)
        return out
    except Exception as exc:
        logger.warning("Could not load offline SHAP importance from %s: %s", p, exc)
        return {}
...
class MetricsCollector:
    ...
    def __init__(self, window_size_seconds: int = 60, shap_importance_path: str = None):
        ...
        # R13: online top-SHAP feature hit counts from anomaly messages
        self.shap_feature_hits = defaultdict(int)

        # R13: offline global SHAP importance generated by shap_explain.py
        self.offline_shap_importance = load_offline_shap_importance(
            shap_importance_path or os.getenv('SHAP_IMPORTANCE_PATH', '/app/models/shap_importance.json')
        )
    ...
    def record_anomaly(self, message: dict):
        ...
        with self._lock:
            ...
            # R13: collect online SHAP explanations if the predictor routed them
            for entry in message.get('top_shap_features') or []:
                if isinstance(entry, dict) and entry.get('feature'):
                    self.shap_feature_hits[str(entry['feature'])] += 1
    ...
    def get_metrics(self) -> dict:
        ...
        with self._lock:
            ...
            return {
                ...
                'shap_online_feature_hits': dict(self.shap_feature_hits),
                'shap_offline_features': len(self.offline_shap_importance)
            }
